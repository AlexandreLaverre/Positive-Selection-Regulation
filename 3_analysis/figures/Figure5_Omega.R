...
all_MLE$peakID <- paste(all_MLE$ID, all_MLE$exp, sep=":")
...
stat <- "omega0" # or "omega2", "omega0", "abs_log2FC" or "meanlog2_mel"
genes <- "all" # or TF
################################################################################
# Merge and bin
if (genes == "all"){
  ...
  labels = c("0-10", "11-25", "26-100", "101-500", "501-1k", ">1k")
}else{
...
nb_peaks <- ggplot(all_genes, aes(x = Freq_bin, y = !!sym(stat))) +
...
pos_genes[[stat]] <- all_genes[match(pos_genes$Gene.name, all_genes$Gene.name),stat]
...
# Boxplot with points
ratio <- ggplot(pos_genes_filtered, aes(x = ratio_bin, y = !!sym(stat))) +
  geom_violin(fill = "gray70", color = NA, alpha = 0.6, trim = TRUE) +
