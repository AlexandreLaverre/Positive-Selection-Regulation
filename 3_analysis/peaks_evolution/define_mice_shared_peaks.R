...
pdf(paste0("/Users/alaverre/Documents/Detecting_positive_selection/results/figures/mice_peaks_overlap.pdf"))
################################################################################
for (TF in TFs){
  message(TF)
  peaks <- list()
  ...
  for (sp in species){
    ...
    BED <- read.table(paste0(path, "peaks_calling/NarrowPeaks/", common_name, "/", sample, "/", TF, ".peaks_UCSC_names.bed"))
    colnames(BED) <- c("chr", "start", "end", "complete_ID")
    row.names(BED) <- BED$complete_ID
    BED[,sp] <- row.names(BED)
    
    for (target in setdiff(species, sp)){
      # Correspondence lifted
      lifted <- read.table(paste0(path, "peaks_overlap/NarrowPeaks/mouse/", TF, "/lifted_peaks/", sp, "_", TF, "_to_", target, "_0.9.bed"), header=F)
      lifted[[target]] <- paste0(lifted$V1, ":", lifted$V2, ":", lifted$V3)
      row.names(lifted) <- lifted$V4
      lifted_peaks[[sp]][[target]] <- lifted
      
      # Overlapping peaks
      overlap <- read.table(paste0(path, "peaks_overlap/NarrowPeaks/mouse/",TF, "/overlap/", sp, "_lifted_overlap_", target, ".txt"), header=T)
  ...
  for (sp in species){
    ...
    write.table(conserved, file = paste0(path, "peaks_overlap/NarrowPeaks/mouse/", TF, "/peaks_by_class/", sp, "_conserved_peaks.txt"), quote=F, col.names = F, row.names = F, sep="\t")
    write.table(gain, file = paste0(path, "peaks_overlap/NarrowPeaks/mouse/",TF, "/peaks_by_class/", sp, "_gain_peaks.txt"), quote=F, col.names = F, row.names = F, sep="\t")
    write.table(loss_in_sp, file = paste0(path, "peaks_overlap/NarrowPeaks/mouse/",TF, "/peaks_by_class/", sp, "_loss_peaks.txt"), quote=F, col.names = F, row.names = F, sep="\t")
  }
}
