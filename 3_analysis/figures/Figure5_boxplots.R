...
AllMerged = paste0(path, "allMLE_drosophila_", method, "_4sub_all_exp.Rds")
...
all_genes_original$MeanSub <- tapply(fullData$Nmut, fullData$Gene, mean) # Mean mutations per gene
...
#for (evol_data in c(gene.expr, Selectome)){
#  evol_data <- get(evol_data)
#}
...
ylab = stat
# Plot: violin (soft, background) + boxplot (emphasized)
