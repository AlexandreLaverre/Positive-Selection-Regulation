...
col_permut=c("forestgreen", "firebrick")
names(col_permut) = c("Positive", "Neutral")

bin = "50"
data = "by_params_independent_SVM_without_backMut_noBin" #"by_deltas"by_params_independent_SVM_without_backMut_noBin
epistasis = ifelse(data=="by_params_independent_SVM_without_backMut_noBin", "without_epistasis", "with_epistasis")
...
delta_total <- read.table(paste0(path, "/deltas/simulated_initial_all_possible_deltaSVM.txt"), h=T, sep="\t", quote="", fill=T, row.names = 1)
...
for (sel in selection){
  ...
  deltas <- read.table(paste0(path, "/deltas/simul_", data, "_", sel, "_observed_deltaSVM.txt"), h=F, sep="\t", quote="", fill=T, col.names = obs_col)
  row.names(deltas) <- deltas$seq_name
  all_delta[[paste0(sel, data)]] <- deltas
  
  # MaxLL
  simul <- read.csv(paste0(path, "/Tests/MLE_summary_simulated_", data, "_", sel, "_", bin, "bins.csv"), h=T, row.names = 1)
  ...
  simul$length <- as.numeric(str_split_i(simul$ID, ":", 3))-as.numeric(str_split_i(simul$ID, ":", 2))
  ...
  simul <- cbind(simul, permut[,c("SVM", "med.expected.deltaSVM", "mean.expected.deltaSVM", "pval.high", "signif", "signif.bin")])
  ...
  #simul$Conclusion <- factor(simul$Conclusion, levels=c("Positive model Decreasing", "Positive model Increasing", "Stabilizing model", "Neutral model"))
  simul$Conclusion <- factor(simul$Conclusion, levels=c("Positive model", "Stabilizing model", "Neutral model"))
...
for (sel in selection){
  ...
  barplot(a[,2]/(a[,2]+a[,1]), las=1, xlab="Nb substitutions", ylab="Proportion signif", main=paste("Permut Test", sel))
  
  a <- table(scenar$Nmut, scenar$Model.bin)
  barplot(a[,2]/(a[,2]+a[,1]), las=1, xlab="Nb substitutions", ylab="Proportion signif", main=paste("LL Ratio Test", sel))
