...
if [ ${sp} = "human" ]; then
	...
	sis_sp_names="Homo_sapiens,Pan_troglodytes,Gorilla_gorilla"
...
if [ ${sp} = "macaca" ]; then
	...
	sis_sp_names="Macaca_mulatta,Macaca_fascicularis,Macaca_nemestrina"
...
if [ ${sp} = "chicken" ]; then
	...
	sis_sp_names="Gallus_gallus,Coturnix_japonica,Meleagris_gallopavo"
	anc_name="birdAnc333"
	chroms=({1..33} "Z" "W" "MT")
...
if [ ${sp} = "rat" ]; then
	...
	sis_species="Rattus_norvegicus,Mus_musculus,Acomys_cahirinus"
...
if [ ${sp} = "cattle" ]; then
	...
	sp_name="Bos_taurus"
	sis_sp_names="Bos_taurus,Bos_indicus,Bos_mutus"
...
if [ ${sp} = "rabbit" ]; then
	spID="GCA_000003625.3_OryCun2.0"
