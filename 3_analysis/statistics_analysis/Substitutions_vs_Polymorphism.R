...
TFS = c("CEBPA", "HNF4A")
sp = "human"
pdf(file=paste0(path, "results/figures/", sp, "_substitutions_polymorphism_FDR_", treshold, "_", minMut, "mut.pdf"), width=8)
par(mfrow=c(2,2))
par(mar=c(9,5,4,2))
...
for (TF in TFS){
  ...
  for (data in names(list_deltaSVM)){
    ...
    pos<- deltaSVM[which(deltaSVM$FDR<treshold),]
    nonPos<- deltaSVM[which(deltaSVM$FDR>=treshold),]
