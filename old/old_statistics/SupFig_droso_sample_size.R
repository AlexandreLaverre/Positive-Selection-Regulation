...
pathFigures <- "/Users/alaverre/Documents/Detecting_positive_selection/cluster/results/final_figures/"
sp = "drosophila"
...
ggplot(df, aes(x = stage, y = sample_size, fill = stage)) +
  ...
  theme(
    ...
    axis.text = element_text(size = 11),   # axis tick text
    axis.title = element_text(size = 11)   # axis labels
  ) +
  labs(title="N=674 experiments", y = "Number of Peaks", x = "")

ggsave(paste0(pathFigures, "/SupFig_droso_sample_size.pdf"), width = 6, height = 4, useDingbats = FALSE)
