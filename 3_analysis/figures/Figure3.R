...
plotLines <- function(all, param, xlab="Directional Selection Strenght (alpha)", max=1) {
  ...
  if (all$Scenario[1] %in% c("Neutral", "Random")){
    ylab <- "False Positive"
  } else {ylab <- "True Positive"}
    
  if (all$Scenario[1] == "Stabilizing"){
...
midpoints <- round((head(breaks, -1) + tail(breaks, -1)) / 2, 0)
...
p1 <- plotLines(all_pos, "Nmut", xlab="Substitution number", max=1) + 
      ...
      labs})
p2 <- plotLines(all_pos, "classdeltaSVM", xlab=expression(Delta*"SVM quantile value"), max=1) +
      scale_x_discrete(labels = function(x) {
      labs <- as.character(x)
      labs[seq_along(labs) %% 2 != 1] <- ""  # show every 2th label
      labs
...
params_pos=c(2, 3, 4, 5, 10, 25, 50, 75)
...
all_stab$classdeltaSVM <- cut(all_stab$deltaSVM, breaks=quantile(all_stab$deltaSVM, probs = seq(0, 1, 0.05)), include.lowest = T)
...
p_stab1 <- plotLines(all_stab, "Nmut", xlab="Substitution number", max=1) + 
            scale_x_discrete(labels = function(x) {
              ...
              labs[seq_along(labs) %% 2 != 1] <- ""  # show every 2th label
              labs})
p_stab2 <- plotLines(all_stab, "classdeltaSVM", xlab="deltaSVM", max=1) + 
            scale_x_discrete(labels = function(x) {
              labs <- as.character(x)
              labs[seq_along(labs) %% 2 != 1] <- ""  # show every 2th label
              labs})
...
ggsave(paste0(pathFigure, "SupFigure_simul_stabilising.pdf"), combined_stab, width = 9.5, height = 11)
ggsave(paste0(pathFigure, "SupFigure_simul_stabilising.png"), combined_stab, width = 9.5, height = 11)
...
pbars <- ggplot(plot_df, aes(x = Scenario, y = Proportion, fill = InferredModel)) +
  ...
  labs(
    x = "Simulated evolutionary scenario",
    y = "Proportion",
    fill = "Inferred model"
  ) +
  theme_minimal(base_size = 16) +
...
ggsave(paste0(pathFigure, "Figure3.pdf"), final_plot, width = 9.5, height = 11)
ggsave(paste0(pathFigure, "Figure3.png"), final_plot, width = 9.5, height = 11)

################################################################################
# Figure 4 
# False positive on delta SVM
breaks <- quantile(all_null$deltaSVM, probs = seq(0, 1, 0.01), na.rm = TRUE)
midpoints <- round((head(breaks, -1) + tail(breaks, -1)) / 2, 0)
all_null$classdeltaSVM <- cut(all_null$deltaSVM, breaks=breaks, include.lowest = T, labels=midpoints)

p_extremeSVM <- plotLines(all_null, "classdeltaSVM", xlab=expression(Delta*"SVM quantile value"), max=0.45) +
  scale_x_discrete(labels = function(x) {labs <- as.character(x) 
  labs[seq_along(labs) %% 3 != 1] <- ""  # show every 2th label
  labs}) 

p_extremeSVM <- p_extremeSVM + theme(legend.position="inside", legend.position.inside = c(0.3, 0.8))
p_extremeSVM
# Ascertainment Bias
...
p_bias <- ggplot(df_plot, aes(x = Fraction, y = Proportion_SVM, group = Test, color = Test)) +
  ...
  labs( title = "", x = "Fraction of dataset (highest SVM)", y = "", color = "" ) +
  ylim(0, max(df_plot$Proportion)*1.2) +
  theme_minimal(base_size = 16) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none")

fig4 <- p_extremeSVM + p_bias + plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size = 18, face = "bold"))
fig4
ggsave(paste0(pathFigure, "Figure4.pdf"), fig4, width = 9.5, height = 4)
ggsave(paste0(pathFigure, "Figure4.png"), fig4, width = 9.5, height = 4)
