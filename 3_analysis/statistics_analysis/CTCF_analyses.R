...
deltaSVM_ML <- deltaSVM_ML[which(deltaSVM_ML$Nmut >= minMut),]
deltaSVM_ML$FDR <- p.adjust(deltaSVM_ML$p_value_purif_pos, method="fdr")
deltaSVM_ML$Conclusion <- ifelse(deltaSVM_ML$FDR > treshold , "Neutral model", deltaSVM_ML$Conclusion)
...
pdf(file=paste0(path, "results/figures/CTCF_", sp, "_number_organs_FDR_", treshold, "_", minMut, "mut_new.pdf"))
par(mfrow=c(2,2))
Delta_org <- list()
for (data in names(list_deltaSVM)){
  deltaSVM = list_deltaSVM[[data]]
  deltaSVMorgNumb<-merge(deltaSVM,organNumb,by=c("chr","start","end"))
  posTFBS<-subset(deltaSVMorgNumb,deltaSVMorgNumb$FDR<treshold)
  nonPosTFBS<-subset(deltaSVMorgNumb,deltaSVMorgNumb$FDR>=treshold)
...
pdf(file=paste0(path, "results/figures/CTCF_", sp, "_proportion_organs_FDR_", treshold, "_", minMut, "mut_new.pdf"), width=8)
for (data in names(list_deltaSVM)){
  ...
  for (i in tissue[[sp]]) {
    ...
    posNumb[i] <- nrow(tissueBasedData[tissueBasedData$FDR<treshold,])
    if (data=="Mine_MaxLL"){Sum_LL_pos[i] <- mean(tissueBasedData[,"LL_pos"])}
    propPos[i] <- posNumb[i]/nrow(tissueBasedData)
    allNumb[i] <- nrow(tissueBasedData)
  }
  Norgan=length(systems[[sp]])
  organProp<-data.frame("organ" = rep(NA,Norgan), "prop" = rep(NA,Norgan))
  organProp$organ <- tissue[[sp]]
  organProp$prop <- propPos
  organProp$posNumb <- posNumb
  organProp$allNumb <- allNumb
  if (data=="Mine_MaxLL"){organProp$Sum_LL_pos <- Sum_LL_pos}
  
  ## fisher test for proportions
  ftest<-data.frame("odds_ratio" = rep(NA,Norgan), "pvalue" = rep(NA,Norgan))
  for (i in c(1:Norgan)) {
    temp<-fisher.test(matrix(c(organProp[i,3],organProp[i,4] ,nrow(deltaSVMorgNumb[deltaSVMorgNumb$FDR<0.1,]),nrow(deltaSVMorgNumb)),nrow = 2,ncol = 2))
  ...
  cor = cor.test(organProp$prop,organProp$allNumb)$estimate
  plot(organProp$prop~organProp$allNumb, ylab="Prop Positive", xlab="Total number")
  text(par("usr")[2]-10000, par("usr")[4]-0.0005, paste("r =", round(cor, 3)), cex=1.2, xpd=TRUE)
  
  if (data=="Mine_MaxLL"){
    organProp <- organProp[order(organProp$Sum_LL_pos),]
    plot(c(1:Norgan), organProp$Sum_LL_pos, ylab="Mean -(Likelihood Positive)", main=data,xlab="", 
         pch=16,cex=2,xaxt="n", cex.lab=1.2, cex.main=1.5, cex.axis=1.2,type="p", col=col[systems[[sp]][organProp$organ]])
    ...
    cor = cor.test(organProp$Sum_LL_pos,organProp$allNumb)$estimate
    plot(organProp$Sum_LL_pos~organProp$allNumb, ylab="Sum_LL_pos", xlab="Total number")
    text(par("usr")[2]-10000, par("usr")[4]-0.0005, paste("r =", round(cor, 3)), cex=1.2, xpd=TRUE)
