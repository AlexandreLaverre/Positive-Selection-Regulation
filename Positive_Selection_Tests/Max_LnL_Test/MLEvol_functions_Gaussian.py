...
def general_plot(all_deltas, obs, svm_distribution, purif, pos, scenario, test, sub_ax1, sub_ax2):
    """
    #To obtain all mutations and substitutions in a distinct plot
    ...
    sub_ax2.hist(all_deltas, bins=100, density=True, alpha=0, label="Substitutions")
    for obs_value in obs:
        sub_ax2.axvline(obs_value, color='red', linestyle='--')

    sub_ax2.set_xlabel("deltaSVM")
    sub_ax2.set_ylabel("Density")

    legend_elements = [mat.lines.Line2D([0], [0], color='red', linestyle='--', linewidth=0.1,
                                        label=f'Sub: m={round(np.mean(obs), 2)}, std={round(np.std(obs), 2)}')]
    sub_ax2.legend(handles=legend_elements)
    sub_ax2.set_title(f'{scenario}')
    """
